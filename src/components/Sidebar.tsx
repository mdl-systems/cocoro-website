...
const navItems = [
    { href: "/", icon: Home, label: "ホーム", badge: null },
        ...
        >
            ...
            <div className="flex items-center gap-3 px-5 py-6 relative">
                ...
                {!collapsed && (
                    <div className="animate-in">
                        ...
                        <div className="text-[10px] text-[#6b6b80] tracking-widest uppercase">
                            AI Platform
                        </div>
                    </div>
                )}
                ...
                >
                    {collapsed ? (
                        <ChevronRight className="w-3 h-3 text-[#6b6b80]" />
                    ) : (
                        <ChevronLeft className="w-3 h-3 text-[#6b6b80]" />
                    )}
                </button>
            </div>
            ...
            {!collapsed && (
                <div className="px-4 mb-4 animate-in">
                    <div
                        className="flex items-center gap-2 px-3 py-2.5 rounded-xl cursor-pointer transition-all duration-200 hover:border-[rgba(255,255,255,0.14)]"
                        style={{
                    ...
                    >
                        ...
                        <span
                            className="ml-auto text-[10px] text-[#6b6b80] px-1.5 py-0.5 rounded"
                            style={{
            ...
            <nav className="flex-1 px-3 space-y-1">
                {!collapsed && (
                    <div className="px-3 mb-2 text-[9px] text-[#6b6b80] tracking-[0.15em] uppercase">
                        メニュー
                    </div>
                )}
                {navItems.map((item) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <Icon
                                className={`w-[18px] h-[18px] flex-shrink-0 transition-colors ${isActive ? "" : "group-hover:text-[#e8e8f0]"
                                    }`}
                            />
                            {!collapsed && (
                                <>
                                    <span
                                        className={`text-[13px] font-medium transition-colors ${isActive ? "" : "group-hover:text-[#e8e8f0]"
                                            }`}
                                    >
                                        {item.label}
                                    </span>
                                            ...
                                            style={{
                                                background:
                                                    item.badge === "AI"
                                                        ...
                                                        : "rgba(139, 92, 246, 0.12)",
                                                color:
                                                    item.badge === "AI" ? "#06b6d4" : "#8b5cf6",
                                                border: `1px solid ${item.badge === "AI"
                                                        ? "rgba(6, 182, 212, 0.2)"
                                                        : "rgba(139, 92, 246, 0.2)"
                                                    }`,
                                            }}
            ...
            <div className="px-4 mb-3">
                <button
                    className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl cursor-pointer transition-all duration-200 hover:translate-y-[-1px] ${collapsed ? "px-0" : "px-4"
                        }`}
                    style={{
            ...
            >
                <div
                    className={`flex items-center gap-3 px-3 py-2 rounded-xl transition-all duration-200 cursor-pointer hover:bg-white/[0.03] ${collapsed ? "justify-center" : ""
                        }`}
                >
                    <div
                    ...
                    {!collapsed && (
                        <Settings className="w-4 h-4 text-[#6b6b80] hover:text-[#a1a1b5] transition-colors flex-shrink-0" />
                    )}
                </div>
            </div>
        </aside>
