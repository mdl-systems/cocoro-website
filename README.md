...
| Core AI SDK | `@mdl-systems/cocoro-sdk`（SSE真のストリーミング対応）|
| AI | cocoro-core（優先）/ OpenAI API（フォールバック）/ Mock |
| データベース | PostgreSQL in production / モックフォールバック（DB未接続時）|
...
| 投稿データ | モックフォールバック（メモリ内配列）| PostgreSQL（pg direct）|
| コミュニティ | モックフォールバック（メモリ内配列）| PostgreSQL（pg direct）|
| AI チャット | Mock SSE（文字ごと送信）| cocoro-core / OpenAI（真のword-by-word stream）|
| AI自律投稿 | シミュレーション | cocoro-agent → PUT /api/agent-task → PostgreSQL 保存 |
